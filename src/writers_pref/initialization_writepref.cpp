...
int main(){
    ...
    int shmid = shmget(READSHMKEY,sizeof(*reader_counter),0666|IPC_CREAT); // creating 4 bytes (size of reader_counter integer) of shared memory 
    ...
    *reader_counter = 0; // ensuring reader_counter is initialised to 0 before read operations
    ...
    int shmid2 = shmget(WRITESHMKEY,sizeof(*writer_counter),0666|IPC_CREAT); // creating 4 bytes (size of writer_counter integer) of shared memory 
